...
function optionsframework_admin_init() 
{
	...
	global $seo_options, $seo_machine;
	$seo_machine = new Options_Machine($seo_options, 'seo');

	global $comp_options, $comp_machine;
...
function optionsframework_add_admin() {
    ...
    $icon_url = FRAMEWORKURL . '/images/settings.png';
    ...
    add_submenu_page('theme-options', THEMENAME, 'Theme SEO', 'edit_theme_options', 'seo-options','optionsframework_seo_page'); // Default
    add_submenu_page('theme-options', THEMENAME, 'Theme Elemets', 'edit_theme_options', 'comp-options','optionsframework_comp_page'); // Default
		
    global $submenu;
    $submenu['theme-options'][0][0] = __('Theme Options', 'themeton');
...
function optionsframework_seo_page(){	
	global $seo_machine;        
	include_once( ADMIN_PATH . 'front-end/options.php' );
}

function optionsframework_comp_page(){	
	global $comp_machine;        
	include_once( ADMIN_PATH . 'front-end/options.php' );
}
...
function of_ajax_callback() 
{
	...
	{			
		if(isset($all['custom_sidebar']))
			$sidebar = $all['custom_sidebar'];
		$data = get_option(BACKUPS);
		$data['custom_sidebar'] = $sidebar;
...
function seo_of_ajax_callback() 
{
	global $seo_machine, $seo_options;

	$nonce=$_POST['security'];
	
	if (! wp_verify_nonce($nonce, 'seo_of_ajax_nonce') ) die('-1'); 
			
	//get options array from db
	$all = get_option(SEO_OPTIONS);
	...
	elseif ($save_type == 'save')
	{
		wp_parse_str(stripslashes($_POST['data']), $seo);
		unset($seo['security']);
		unset($seo['of_save']);
		update_option(SEO_OPTIONS, $seo);
...
function comp_of_ajax_callback() 
{
	global $comp_machine, $comp_options;

	$nonce=$_POST['security'];
	
	if (! wp_verify_nonce($nonce, 'comp_of_ajax_nonce') ) die('-1'); 
			
	//get options array from db
	$all = get_option(COMP_OPTIONS);
	...
	if($save_type == 'upload')
	{
		...
		$uploaded_file = wp_handle_upload($filename,$override);
			...
			update_option(COMP_OPTIONS, $upload_image ) ;
	...
	elseif($save_type == 'image_reset')
	{
			...
			$delete_image = $all; //preserve rest of data
			$delete_image[$id] = ''; //update array key with empty value	 
			update_option(COMP_OPTIONS, $delete_image ) ;
	...
	{			
		...
		update_option(COMP_OPTIONS, $data);
	...
	elseif ($save_type == 'save')
	{
		wp_parse_str(stripslashes($_POST['data']), $comp);
		unset($comp['security']);
		unset($comp['of_save']);
		update_option(COMP_OPTIONS, $comp);
