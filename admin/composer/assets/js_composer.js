...
jQuery(document).ready(function($) {
    ...
    */
    $( "#wpb_visual_composer .dropable_el, #wpb_visual_composer .dropable_column" ).draggable({
        ...
        start: function(event, ui) {
            renderCorrectPlaceholder(event, ui);
        }
    });
    initDroppable();
	...
	*/
    ...
    if ( $("#wpb_visual_composer").length == 1 ) {
        $('div#titlediv').after('<p class="composer-switch"><a class="wpb_switch-to-composer button-primary" href="#">Theme Elements</a></p>'+jQuery('#wp-content-media-buttons').html());
        ...
        $('.wpb_switch-to-composer').click(function(e){
            ...
            // }
                ...
                $(this).html('Theme Elements');
                $("p.composer-switch").next('a').hide();
            }
        });
        ...
        //wpb_shortcodesToVisualEditor();
            ...
            postdivrich.show();
            visualcomposer.hide();
            $('.wpb_switch-to-composer').html('Theme Elements');
            $("p.composer-switch").next('a').hide();
        }
    }
    jQuery(window).load(function() {
        ...
        $("p.composer-switch").next('a').click(function(){
            var iTB = setInterval(function(){
                if ($('#TB_window').html()){
                    $('#TB_window').addClass('theme_elements_TB_window');
                }
            }, 100);
        });
    });
...
function wpb_processScroll() {
    ...
    //START - Customize
    $tt_menu=jQuery('.nav-collapse li.dropdown.open .dropdown-menu');
    $tt_win_height=$win.height()-75;
    $tt_menu_height=$tt_menu.height();
    //END   - Customize
    if($tt_menu!=null){
        if($tt_win_height<$tt_menu_height){
            $tt_menu.attr("style","height: "+$tt_win_height+"px; overflow-x: hidden; width: 210px;");
        }else{
        //                $tt_menu.attr("style","height: auto; overflow-x: visible; width: auto;");
        }
    }
}
...
function showEditForm(element) {
    ...
    jQuery('#visual_composer_edit_form').html(load_img + ' ' +jQuery('#wpb_vc_loading').val()).show().css({
        "padding-top" : 60
    });
    ...
    // since 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
    jQuery.post(ajaxurl, data, function(response) {
        jQuery('#visual_composer_edit_form').html(response).css({
            "padding-top" : 0
        });
...
/* START - ThemeTon Custom Callbacks
---------------------------------------------------------- */

/* Progress Callbacks
---------------------------------------------------------- */
function ttProgressInitCallBack(element) {
//    var el = element.find('.wpb_vc_param_value.color');
//    var class_to_set = el.val();
//    el.closest('.wpb_element_wrapper').addClass(class_to_set);
}
function ttSave(element) {
    var el,class_to_set,value_to_set;
    switch(element.attr('data-element_type')){
        case"tt_table":{
            el = element.find('.wpb_vc_param_value.table_name');
            class_to_set = el.val();
            ...
            switch(class_to_set){
                case'default_table':
                    ...
                    break;
                case'striped_table':
                    class_to_set="table table-striped";
                    break;
                case'bordered_table':
                    class_to_set="table table-bordered";
                    break;
                case'condensed_table':
                    class_to_set="table table-condensed";
                    break;
                case'combine_them_all':
                    class_to_set="table table-striped table-bordered table-condensed";
                    break;
            }
            
            // START - temp bug repair
            el = element.find('.wpb_vc_param_value.table_content');
            value_to_set = el.val();
            value_to_set=value_to_set.replace(/>,/gi,">");
            value_to_set=value_to_set.substr(value_to_set.indexOf("<table"));
            el.val(value_to_set);
            // END   - temp bug repair
            
            el = element.find('.wpb_vc_param_value.table_content');
            value_to_set = el.val();
            jQuery(value_to_set).removeClass('table table-striped table-bordered table-condensed');
            el.closest('.wpb_element_wrapper').children().html(value_to_set);
            el.closest('.wpb_element_wrapper').find('table').addClass(class_to_set);

            break;
        }
        case"tt_progress":{
            el = element.find('.wpb_vc_param_value.progress_name');
            class_to_set = el.val();
            switch(class_to_set){
                case'basic':
                    class_to_set=" progress";
                    break;
                case'striped':
                    class_to_set=" progress progress-striped";
                    break;
                case'animated':
                    class_to_set=" progress progress-striped active";
                    break;
            }
            el.closest('.wpb_element_wrapper').find('.progress').removeClass('progress-striped active').addClass(class_to_set);

            el = element.find('.wpb_vc_param_value.progress_size');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.bar').css('width', value_to_set+'%');
            
            break;
        }
        case"tt_hero_unit":{
            el = element.find('.wpb_vc_param_value.hero_unit_title');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.tt_title').html(value_to_set);

            el = element.find('.wpb_vc_param_value.hero_unit_tagline');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.tt_tagline').html(value_to_set);

            el = element.find('.wpb_vc_param_value.hero_unit_button_url');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.tt_btn').children('a').attr('href',value_to_set);
            
            el = element.find('.wpb_vc_param_value.hero_unit_button_text');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.tt_btn').children('a').html(value_to_set);
            
            break;
        }
        case"tt_popover":{
            el = element.find('.wpb_vc_param_value.popover_title');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.well').attr('data-original-title',value_to_set);

            el = element.find('.wpb_vc_param_value.popover_content');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.well').attr('data-content',value_to_set);

            el = element.find('.wpb_vc_param_value.popover_text');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.well').html(value_to_set);
            
            break;
        }
        case"tt_tooltip":{
            el = element.find('.wpb_vc_param_value.tooltip_text');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.target_tooltip').html(value_to_set);

            el = element.find('.wpb_vc_param_value.tooltip_content');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.target_tooltip').attr('data-original-title',value_to_set);
            
            break;
        }
        case"tt_space":{
            el = element.find('.wpb_vc_param_value.height');
            value_to_set = el.val();
            el.closest('.wpb_element_wrapper').find('.tt_space').css("margin-top", value_to_set);
           
            break;
        }
        case"vc_cta_button":{
            //            el = element.find('.wpb_vc_param_value.show_button');
            //            value_to_set = el.val();
            //            
            //            if(value_to_set=='show'){
            //                el.closest('.wpb_element_wrapper').css('background-image', 'url('+tt_theme_uri+'admin/composer/assets/vc/cta_block.png)');
            //            }else{
            //                el.closest('.wpb_element_wrapper').css('background-image', '');
            //            }
...
function wpbButtonInitCallBack(element) {
    ...
    //
    element.find('button.title').attr({
        "class" : "wpb_vc_param_value title textfield btn " + el_class
    });
    element.find('button.title').css('float','none').css('width','auto');
