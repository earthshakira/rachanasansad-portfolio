...
class WPBakeryVisualComposerSetup extends WPBakeryVisualComposerAbstract {
    ...
    public function init($settings) {
        ...
        if ( basename(dirname(self::$config['APP_ROOT'])) == 'plugins' ) {
            $this->composer->setPlugin();
            $this->setUpPlugin();
        } elseif ( basename(dirname(dirname(dirname(self::$config['APP_ROOT'])))) == 'themes' ) {
            $this->composer->setPlugin();
            $this->setUpPlugin();
...
class WPBakeryVisualComposerSetupAdmin extends WPBakeryVisualComposerSetup {
    ...
    public function editScreen_js() {
        wp_register_script('bootstrap-js',  get_template_directory_uri() . '/js/bootstrap.min.js', false, WPB_VC_VERSION, true);
//        wp_register_style( 'bootstrap-css', get_template_directory_uri() . '/css/bootstrap.css',   false, WPB_VC_VERSION, false );
        wp_register_style( 'bootstrap-admin-css', $this->assetURL( 'bootstrap/css/bootstrap.css' ), false, WPB_VC_VERSION, 'screen' );
...
//        wp_register_style( 'bootstrap', $this->assetURL( 'bootstrap/css/bootstrap.css' ), false, WPB_VC_VERSION, false );
    ...
    public function jsComposerEditPage() {
        global $comp;
        if(isset($comp['enable_composer']) && $comp['enable_composer']) {
            $pt_array = $this->composer->getPostTypes();
            foreach ($pt_array as $type=>$pt) {
                add_meta_box( 'wpb_visual_composer', __('Theme Elements', "js_composer"), Array($this->composer->getLayout(), 'output'), $type, 'normal', 'high');
            }
        }
    }
    ...
    public function composerSettings() {
        ...
        if ( current_user_can('manage_options') && $this->composer->isPlugin()) {
            add_options_page(__("Theme Elements Settings", "js_composer"), __("Theme Elements", "js_composer"), 'install_plugins', "wpb_vc_settings", array($this, "composerSettingsMenuHTML"));
        }
    }
