...
class WPBakeryShortCode_VC_Tab extends WPBakeryShortCode {
    public function content( $atts, $content = null ) {
        ...
        $output .= '<div id="tab-'.sanitize_title($title).'" class="wpb_tab">';
        $output .= wpb_js_remove_wpautop($content);
        $output .= '</div>' . $this->endBlockComment('.wpb_tab');
        return $output;
    }
...
class WPBakeryShortCode_VC_Tabs extends WPBakeryShortCode {
    ...
    public function content($atts, $content =null)
    {
        ...
        $output .= '<div class="'.$element.' wpb_content_element '.$width.$el_class.'" data-interval="'.$interval.'">';
        $output .= '<div class="wpb_wrapper wpb_tour_tabs_wrapper">';
        $output .= ($title != '' ) ? '<h2 class="wpb_heading '.$element.'_heading">'.$title.'</h2>' : '';
        $output .= $tabs_nav;
        $output .= wpb_js_remove_wpautop($content);
        if ( 'vc_tour' == $this->shortcode) {
            $output .= '<div class="wpb_tour_next_prev_nav"> <span class="wpb_prev_slide"><a href="#prev" title="'.__('Previous slide').'">'.__('Previous slide').'</a></span> <span class="wpb_next_slide"><a href="#next" title="'.__('Next slide').'">'.__('Next slide').'</a></span></div>';
        }
