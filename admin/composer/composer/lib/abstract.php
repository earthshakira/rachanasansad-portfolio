...
abstract class WPBakeryVisualComposerAbstract {
    ...
    public function assetURL($asset) {
        //return plugins_url( self::$config['APP_DIR'] . self::$config['ASSETS_DIR'] . $asset, self::$config['APP_DIR'] );
        return get_template_directory_uri().'/admin/'. self::$config['APP_DIR'] . self::$config['ASSETS_DIR'] . $asset;
    }
}
