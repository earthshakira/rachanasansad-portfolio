...
jQuery(document).ready(function($){
	...
	// SEO Display last current tab	
	if ($.cookie("seo_of_current_opt") === null) {
		$('.seo_options .group:first').fadeIn('fast');	
		$('.seo_options #of-nav li:first').addClass('current');
	} else {
		...
		if(hooks) {
                    $.each(hooks, function(key, value) { 
                            ...
                            if ($.cookie("seo_of_current_opt") == '#of-option-'+ value) {
                                    $('.seo_options .group#of-option-' + value).fadeIn();
                                    $('.seo_options #of-nav li.' + value).addClass('current');
                            }
	...
	// COMP Display last current tab	
	if ($.cookie("comp_of_current_opt") === null) {
		$('.comp_options .group:first').fadeIn('fast');	
		$('.comp_options #of-nav li:first').addClass('current');
	} else {
		...
		if(hooks) {
                    $.each(hooks, function(key, value) { 
                            ...
                            if ($.cookie("comp_of_current_opt") == '#of-option-'+ value) {
                                    $('.comp_options .group#of-option-' + value).fadeIn();
                                    $('.comp_options #of-nav li.' + value).addClass('current');
                            }
	...
	//Current Menu Class
	$('.theme_options #of-nav li a').click(function(evt){
	...
	//Current Menu Class
	$('.seo_options #of-nav li a').click(function(evt){
	// event.preventDefault();
		...
		$.cookie('seo_of_current_opt', clicked_group, {expires: 7, path: '/'});
	...
	//Current Menu Class
	$('.comp_options #of-nav li a').click(function(evt){
	// event.preventDefault();
		...
		$.cookie('comp_of_current_opt', clicked_group, {expires: 7, path: '/'});
	...
	$('#of_save').live('click',function() {
		...
		var serializedReturn = $('#of_form :input[name][name!="security"][name!="of_reset"]').serialize();
                ...
                if(jQuery(this).closest('#of_container').hasClass('theme_options'))
                    var action = 'of_ajax_post_action';
                else if(jQuery(this).closest('#of_container').hasClass('seo_options'))
                    var action = 'seo_of_ajax_post_action';
				else if(jQuery(this).closest('#of_container').hasClass('comp_options'))
                    var action = 'comp_of_ajax_post_action';
		...
		if (answer){
			...
			$('.ajax-reset-loading-img').fadeIn();
                        ...
                        if(jQuery(this).closest('#of_container').hasClass('theme_options'))
                            ...
                            action = 'seo_of_ajax_post_action';
						else if(jQuery(this).closest('#of_container').hasClass('comp_options'))
                            action = 'comp_of_ajax_post_action';
