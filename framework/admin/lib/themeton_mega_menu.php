<?php
/*Mega-Menu-Start*/
class TT_Menu_Frontend extends Walker_Nav_Menu  {
    /**
    ...
    global $wp_query;
        ...
        if($depth==1  && $this->menutype=="widget")
            ...
            $output .= "\n$indent<{$element} class=\"children $widget_class \">\n"; }
    }
    ...
    function start_el(&$output, $item, $depth, $args) {
        ...
        $menu_columns=get_post_meta( $item->ID, 'mega-menu-columns-'.$item->ID,true);
...
class TT_MegaMenu_Nav_Menu extends Walker_Nav_Menu {
    /**
            ...
            <dl class="menu-item-bar">
                <dt class="menu-item-handle">
                    ...
                    <span class="item-controls">
                        ...
                        <span class="item-type item-type-tt-column"><?php _e('Column','tt-framework'); ?></span>
                        <span class="item-type item-type-hmenu"><?php _e('(Mega Menu)','tt-framework'); ?></span>
                        <span class="item-type item-type-tt-widget"><?php _e('Widget','tt-framework'); ?></span>

                        
                        <a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php _e('Edit Menu Item','tt-framework'); ?>" href="<?php
                            echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                        ?>"><?php _e( 'Edit Menu Item' ,'tt-framework'); ?>
                        </a>
                    </span>
                </dt>
            </dl>
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
                ...
                <p class="field-link-target description description-thin">
                    <label for="edit-menu-item-target-<?php echo $item_id; ?>">
                        <?php _e( 'Link Target','tt-framework' ); ?><br />
                        <select id="edit-menu-item-target-<?php echo $item_id; ?>" class="widefat edit-menu-item-target" name="menu-item-target[<?php echo $item_id; ?>]">
                            <option value="" <?php selected( $item->target, ''); ?>><?php _e('Same window or tab','tt-framework'); ?></option>
                            <option value="_blank" <?php selected( $item->target, '_blank'); ?>><?php _e('New window or tab','tt-framework'); ?></option>
                        </select>
                    </label>
                </p>
                <p class="field-css-classes description description-thin">
                    <label for="edit-menu-item-classes-<?php echo $item_id; ?>">
                        <?php _e( 'CSS Classes (optional)','tt-framework' ); ?><br />
                        <input type="text" id="edit-menu-item-classes-<?php echo $item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                    </label>
                </p>
                <p class="field-xfn description description-thin">
                    <label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
                        <?php _e( 'Link Relationship (XFN)','tt-framework' ); ?><br />
                        <input type="text" id="edit-menu-item-xfn-<?php echo $item_id; ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                    </label>
                </p>
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo $item_id; ?>">
                        <?php _e( 'Description','tt-framework' ); ?><br />
                        ...
                        <span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.','tt-framework'); ?></span>
                    </label>
                </p>

                <div class="menu-item-actions description-wide submitbox">
                    <p class="tt_megamenu_box clearfix">
                    ...
                    <p class="tt_megamenu_row_box clearfix">
                    ...
                    <?php if( 'custom' != $item->type ) : ?>
                        <p class="link-to-original">
                            <?php printf( __('Original:','tt-framework').' %s', '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    <a class="item-cancel submitcancel" id="cancel-<?php echo $item_id; ?>" href="<?php	echo add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) );
                            ?>#menu-item-settings-<?php echo $item_id; ?>"><?php _e('Cancel','tt-framework'); ?>
                    </a>
                </div>
...
function hmenu_scripts()
{
    ...
    {
        wp_enqueue_script( 'themeton_mega_menu' , get_template_directory_uri() . '/framework/js/admin/themeton_mega_menu.js',array('jquery', 'jquery-ui-sortable'), false, true ); 
    }
}
...
function update_menu($menu_id, $menu_item_db)
{
    ...
    update_post_meta( $menu_item_db, 'mega-menu-columns-'.$menu_item_db, $_POST['mega-menu-columns-'.$menu_item_db] );
...
function TT_MegaMenu_AJAX_Menu()
{	
    ...
    if ( ! empty( $menu_items ) ) {
            $args = array(
                    ...
                    'link_after' => '',
                    'link_before' => '',
                    'walker' => new TT_MegaMenu_Nav_Menu,
            );
...
add_action('wp_ajax_TT_MegaMenu_AJAX_Menu', 'TT_MegaMenu_AJAX_Menu');
/*Mega-Menu-End*/