...
jQuery(function($){
	var temp, type ;
	$(".tt_megamenu_box input[type=checkbox]").live("click",function(){
		temp = $(this);
		if(temp.is(":checked"))
		{
	...
	$(".enable_textbox").live("click",function(){
	 ...
	 $("#menu-to-edit").find(".menu-item").each(function(){
		...
		{
			 $(this).find(".tt_megamenu_box").hide();
			  $(this).find(".item-type-hmenu").hide();
			 if($(this).find(".tt_megamenu_box input[type=checkbox]").is(":checked"))
			 {
				 $(this).find(".tt_megamenu_box input[type=checkbox]").removeAttr("checked");
			 }
		}
		 else
		{
			...
			if(type="column")
			{
			 ...
			 $(this).find(".tt_megamenu_box").show();
			  $(this).find(".tt_megamenu_row_box").hide();
	...
	$(".tt_megamenu_box input[type=checkbox]").each(function(){
	...
	type = $(this).find(".tt_megamenu_box select").val();
	...
	var flag=false,dragel = ui;
	   ...
	   if( $(pindex).find(".tt_megamenu_box input[type=checkbox]").is(":checked") )
	   flag = true;
	   else
	   flag = false;
	 ...
	 temp.find(".menu-item").each(function(){
		 ...
		 if(!$(this).hasClass("menu-item-depth-0"))
		{
			...
			$(this).find(".tt_megamenu_row_box").show();
			$(this).find(".item-type-tt-column").show();
			}
			else
			{
				 $(this).find(".tt_megamenu_box").hide();
		 ...
		 else
		{
			 $(this).find(".tt_megamenu_row_box").hide();
			 $(this).find(".tt_megamenu_box").show();
			 $(this).find(".item-type-tt-column").hide();
			 // $(this).find(".item-type-hmenu").show();
			 
			  if($(this).find(".tt_megamenu_row_box input[type=checkbox]").is(":checked"))
			 {
				 $(this).find(".tt_megamenu_row_box input[type=checkbox]").removeAttr("checked");
			 }
...
function set_column_tree(parent)
{
	...
	if(parent.find(".tt_megamenu_box input[type=checkbox]").is(":checked"))
	{
	...
	$("#menu-to-edit").find(".menu-item").each(function(){
		...
		if(test>0)
		{
			...
			if(id==test)
			{
				obj.find(".item-type-tt-column").show();
				obj.find(".tt_megamenu_row_box").show();
			}
		}
...
function restruct_tree(parent)
{
	...
	$("#menu-to-edit").find(".menu-item").each(function(){
		...
		if(test>0)
		{
			...
			if(id==test)
			{
				obj.find(".item-type-tt-column").hide();
				obj.find(".tt_megamenu_row_box").hide();
			}
		}
